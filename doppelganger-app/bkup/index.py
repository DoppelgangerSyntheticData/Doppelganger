...
#1.1 col names
orig_cols = list(df_le.columns)
new_cols = []
for col in orig_cols:
    new_cols.append(col.strip().replace('  ', ' ').replace(' ', '_').lower())
df_le.columns = new_cols
df_le.rename(columns={'thinness_1-19_years':'thinness_10-19_years'}, inplace=True)

#1.2 missing values
df_le_desc1=df_le.describe().iloc[:, 1:].transpose().round(0)
df_le_desc2=df_le.info()
df_le_desc3=df_le.isnull().sum()/len(df_le)*100

#1.3 drop high bad data columns altogether
df_le.drop('bmi',inplace=True, axis=1)

#1.4  Fill missing data by heiristic
# since this is time series data assorted by country, the best course of action would be to interpolate the data by country
# imputation by year may be the best possible method here. Imputation of each year's mean is done below.
imputed_data = []
for year in list(df_le.year.unique()):
    year_data = df_le[df_le .year == year].copy()
    for col in list(year_data.columns)[3:]:
        year_data[col] = year_data[col].fillna(year_data[col].dropna().mean()).copy()
    imputed_data.append(year_data)
df_le = pd.concat(imputed_data).copy()
imputed_data=[]

#1.5  handling outliers
wins_df=pd.read_csv("data/life_expectancy_data_winz.csv")
    ...
    return dcc.Tabs(id="tabs", children=[
                                dcc.Tab(label='Data Generation', children=[
                                    html.Div([html.H1("Dataset Introduction", style={'textAlign': 'center', 'width':'1024px', 'overflow-x':'auto', 'white-space': 'nowrap'}),
                                              ...
                                              dash_table.DataTable(
                                                id='table2',
                                                columns=[{"name": i, "id": i} for i in wins_df.describe().reset_index().columns],
                                                data=wins_df.describe().reset_index().to_dict("rows"),
                                              ),
                                ...
                                dcc.Tab(label='Data Validation', children=[
                                    html.Div([html.H1("XGBoost Validator", style={"textAlign": "center"}),
                                              ...
                                              dash_table.DataTable(
                                                  ...
                                                  columns=[{"name": i, "id": i} for i in
                                                           df_le.describe().reset_index().columns],
                                                  data=df_le.describe().reset_index().to_dict("rows"),
                                              ),
                                ...
                                dcc.Tab(label='Data Evaluation', children=[
                                    html.Div([html.H1("Machine Learning", style={"textAlign": "center"}),
                                              html.H2("Random Forest Evaluator Performance", style={"textAlign": "left"}),
                                              ...
                                              html.H2("Performance Metrics Regression Prediction",
                                                      style={"textAlign": "left"}), html.P("In this example I used the Performance Metrics dataset to predict the number of likes I post can get. Training a Random Forest Regressor with 500 estimetors right now online lead an accuracy (%) in the Training set equal to: "),
                                              str(train_acc),
                                ...
                                dcc.Tab(label='Data Summary / Download', children=[
                                    html.Div([html.H1("Synthetic Data Summary", style={"textAlign": "center"}),
                                              html.H2("Evaluator vs. Validator", style={"textAlign": "left"}),
                                              dcc.Dropdown(id='my-dropdowntest',
                                                           options=[{'label': 'Tesla', 'value': 'TSLA'},
                                                                    {'label': 'Apple', 'value': 'AAPL'},
                                                                    {'label': 'Facebook', 'value': 'FB'},
